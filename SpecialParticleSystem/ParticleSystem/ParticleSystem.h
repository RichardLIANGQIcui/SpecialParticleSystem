...
private:
	...
	//ParticleEmitter emitter; //每一个粒子系统有自己的一个emitter -> emit
	ParticleMaster master; //每个粒子系统有一个自己的master  -> update
	ParticleRenderer renderer; //每个粒子系统有一个自己的renderer -> render
...
public:
	...
	void generateParticles(const vec3& systemCenter);  //主循环中 每帧调用
