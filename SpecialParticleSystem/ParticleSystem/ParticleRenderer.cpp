...
void ParticleRenderer::updateModelViewMatrix(const vec3& position, float rotation, float scale, mat4 view, float vboData[])
{
	...
	// 修正最终的modelview矩阵的旋转部分 即左上角3x3的矩阵化为单位矩阵
	...
	*(p + 10) = *(q + 10);
	...
	mat4 modelView = mutiMatrixBySimd(view, model);
...
void ParticleRenderer::render(const vector<Particle>& particles, Camera camera)
{
